...
/** Emblema + wordmark "Áquila Condomínios", igual ao header da landing page. */
  ...
  return (
    <div className={cn("flex items-center gap-2.5", className)}>
      <img
        ...
        alt={compact ? "Áquila Condomínios" : ""}
        className="h-auto w-11 shrink-0 dark:hidden"
      />
      <img
        src="/brand/emblema-dark.png"
        alt={compact ? "Áquila Condomínios" : ""}
        className="hidden h-auto w-11 shrink-0 dark:block"
      />
      {!compact && (
        <span className="font-brand leading-none text-foreground">
          <b className="block text-lg font-bold tracking-[0.06em]">ÁQUILA</b>
          <span className="mt-0.5 block text-[8px] font-semibold tracking-[0.32em]">
            CONDOMÍNIOS
          </span>
        </span>
      )}
  ...
  return (
    <div className="relative hidden overflow-hidden border-r bg-background p-12 lg:flex lg:flex-col lg:justify-between">
      ...
      <div className="relative z-10 space-y-4">
        <h1 className="font-heading text-4xl font-black leading-tight text-foreground">
          {title}
        </h1>
      ...
      {video && !reduzirMovimento ? (
        ...
        <div className="relative mx-auto w-full max-w-xs">
          ...
          <img
            ...
            alt="Mascote da Áquila Condomínios: uma águia sorridente de camisa polo azul acenando"
            className="relative w-full drop-shadow-[0_18px_24px_hsl(213_72%_16%/0.12)]"
          />
        </div>
      )}
      <p className="text-sm text-muted-foreground">
        © {new Date().getFullYear()} Áquila Condomínios
      </p>
    </div>
  );
}
